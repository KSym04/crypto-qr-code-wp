...
Save your wallets once on the settings page and reuse them anywhere with a shortcode or a widget. In the block editor, paste the shortcode into a Shortcode block. Each cryptocurrency QR code is generated entirely in your visitor's browser, so nothing is written to your server and there are no external or third party requests.
...
2. Several wallet addresses on a page, with the QR code tooltip open for an ADA wallet.
...
= 1.3.2 =
Shortcodes now work when a wallet heading has quotes or brackets, and the settings page confirms each save. Deleting the plugin now removes its saved wallets and settings. Tested with WordPress 7.1.2.
